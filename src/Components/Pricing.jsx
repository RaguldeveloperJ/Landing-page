import React from 'react'
import { CheckCircle2 } from 'lucide-react'
import { pricingOptions } from '../constants'

const Pricing = () => {
  return (
    <>
     <div className='mt-20' id="Pricing">
        <h2 className='text-3xl sm:text-5xl lg:text-5xl mt-10 lg:mt-20 tracking-wide text-center'>Pricing</h2>
        <div className='flex flex-wrap '>
         {
            pricingOptions.map((items,index)=>{
                return(
                    <div key={index} className='w-full sm:w-1/2 lg:w-1/3 p-2'>
                       <div className="p-10 border border-neutral-700 rounded-xl">
                        <p className='text-3xl mb-8 '>
                            {items.title} {""}
                            {items.title === "Pro" && (
                                <span className='bg-[linear-gradient(to_right,#f97316,#991b1b)] text-transparent bg-clip-text text-xl mb-4'>(Most Popular)</span>
                            )}
                        </p>
                        <p className="mb-8">
                            <span className='text-3xl mt-6 mr-2 '>{items.price}</span>
                            <span className='text-neutral-400 tracking-tight'>/Month</span>
                        </p>
                        
                            <ul>
                                {items.features.map((feature,index)=>{
                                    return(
                                        <li key={index} className='mt-8 flex items-center'>
                                           <CheckCircle2/>
                                           <span className='ml-2'>{feature}</span>
                                        </li>
                                    )
                                })}
                            </ul>
                            <a href="#" className='inline-flex justify-center items-center text-center w-full h-12 p-5 mt-20 tracking-tight text-xl hover:bg-orange-900 border border-orange-900 rounded-lg transition duration-200 '>Subscribe</a>
                        
                       </div>
                    </div>
                )
            })
         }
        </div>
     </div>
    </>
  )
}

export default Pricing